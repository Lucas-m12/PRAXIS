...
class EstoqueModel extends CI_Model{
	...
	public function relatorioEstoque(){
		...
		return $this->db->query("SELECT ME.ESTOQUE_ATUAL, MP.DESC_PROGRAMA, P.DESC_PRODUTO, MUM.SIGLA_UNIDADE_MEDIDA FROM MM_ESTOQUE ME INNER JOIN MM_PROGRAMAS MP USING(ID_PROGRAMA) INNER JOIN MM_PRODUTOS P USING(ID_PRODUTO) INNER JOIN MM_UNIDADES_MEDIDA MUM USING(ID_UNIDADE_MEDIDA) WHERE ME.STATUS = ?", array(
			1
		))->result_array();
