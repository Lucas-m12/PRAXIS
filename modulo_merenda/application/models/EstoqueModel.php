...
class EstoqueModel extends CI_Model{
	...
	public function pesquisarEstoqueEscola($programa = "", $produto = "", $escola = ""){
		...
		return $this->db->query("SELECT MP.DESC_PROGRAMA, MP.PROGRAMA, P.DESC_PRODUTO, MUM.SIGLA_UNIDADE_MEDIDA, MEE.ESTOQUE_ATUAL, UE.NOME_ESCOLA FROM MM_ESTOQUE_ESCOLA MEE INNER JOIN MM_PROGRAMAS MP USING(ID_PROGRAMA) INNER JOIN MM_PRODUTOS P USING(ID_PRODUTO) INNER JOIN MM_UNIDADES_MEDIDA MUM USING(ID_UNIDADE_MEDIDA) INNER JOIN UNIDADE_ENSINO_00 UE USING(ID_ESCOLA) WHERE MEE.ID_PROGRAMA LIKE ? AND MEE.ID_PRODUTO LIKE ? AND MEE.ID_ESCOLA LIKE ?", array(
			"%".$programa."%",
			"%".$produto."%",
			"%".$escola."%"
		))->result_array();
