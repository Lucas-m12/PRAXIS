...
class ProdutoModel extends CI_Model{
	...
	public function pesquisarProduto($nome = '', $tipo = '', $categoria = ''){
		...
		return $this->db->query("SELECT MP.ID_PRODUTO, 
			MP.DESC_PRODUTO, 
			MP.STATUS, 
			MTP.TIPO_PRODUTO, 
			MCP.DESC_CATEGORIA, 
			MUM.SIGLA_UNIDADE_MEDIDA 
			FROM MM_PRODUTOS MP 
			INNER JOIN MM_TIPO_PRODUTOS MTP 
			ON MP.ID_TIPO = MTP.ID_TIPO 
			INNER JOIN MM_CATEGORIA_PRODUTO MCP 
			ON MP.ID_CATEGORIA = MCP.ID_CATEGORIA 
			INNER JOIN MM_UNIDADES_MEDIDA MUM 
			ON MP.ID_UNIDADE_MEDIDA = MUM.ID_UNIDADE_MEDIDA 
			WHERE MP.ID_TIPO LIKE ? AND MP.ID_CATEGORIA LIKE ? AND MP.DESC_PRODUTO LIKE ?", 
			array(
				"%".$tipo."%",
				"%".$categoria."%",
				"%".$nome."%"
